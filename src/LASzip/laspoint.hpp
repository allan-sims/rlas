...
#pragma warning(pop)
  ...
  // rlas patch: drop [[deprecated]] — rlas intentionally uses this legacy
  // accessor for format 1-5 points where scan_angle_rank is the canonical
  // field. CRAN flags deprecation warnings from vendored code as significant.
  ...
  // rlas patch: drop [[deprecated]] (see matching comment on get_scan_angle_rank).
  ...
  // rlas patch: 3.5.0 dropped the explicit get_/set_extended_classification
  // accessors (the unified get_/set_classification now handle both paths).
  // rlasstreamer's SMART_POPULATOR macro and writeLAS.cpp still call them by
  // name, so restore thin wrappers that forward to the unified accessors.
  inline U8 get_extended_classification() const {
    return extended_classification;
  };
  inline void set_extended_classification(U8 value) {
    set_classification(value);
  };
